...
// Constantes para la comunición IOT
const char* UBIDOTS_TOKEN = "BBFF-futcmgmyVLvU4ZR4jEpRQox0KecPZc";    // Put here your Ubidots TOKEN
const char* WIFI_SSID = "JESSENIA";   // Put here your Wi-Fi SSID
const char* WIFI_PASS = "rosita2000@";        // Put here your Wi-Fi password
...
unsigned long const SendData = 8000;
...
const float SensorOffset = 0.18;     // in V taken from datasheet 0.18
...
const float RC = 0.32985; //0.32985;
const float K = 1.15;
const unsigned long Sampling = 10;
float RealPresure = 0;
float LastPresure = 0;
float HighOutput = 0;
float Output = 0;
...
// Objeto pulsumetro oximetro ************
PulseOximeter pox;

// Variables y objetos para el modo de medición de temperatura **********
double temp_amb;
double temp_obj;
...
void Mode1Setup()
{
  ...
  Serial.print("Initializing pulse oximeter.."); // Send a ACK on monitor serial
  // Clear the buffer *******
...
void SendMode1Data()
{
  ubidots.add("bpm", HeartRate);
  ubidots.add("oxigeno", SpO2);
}
...
void SendMode2Data()
{
  ubidots.add("temp_ambiente", temp_amb);
  ubidots.add("temp_objeto", temp_obj);
}
...
void Mode3Setup()
{
  ...
  display.clearDisplay();
  display.setCursor(0, 0);
  display.print("Medición de presión");
  display.display();
  delay(1000);
...
void SendMode3Data()
{
  ubidots.add("P_SIS", SIS);
  ubidots.add("P_DIA", DIA);
}
...
void setup() {
  ...
  display.clearDisplay();
  // Inicialización de los modos de funcionamiento del programa ******
...
void loop() {
  ...
  if (millis() - SendDataTime > SendData) {
    ...
    switch (SelectMode)
    {
      case 0:
        SendMode1Data();
        ubidots.send("ESP8266_Vital_Monitor");
        pox.begin();
        break;
      case 1:
        SendMode2Data();
